...
- P2 默认延期，除非用户明确要求：永久群聊、多文件/文件夹传输、断点续传、消息撤回、已读回执、加密、托盘、通知、开机自启、跨网段手动添加 IP。本轮用户已明确要求已读回执、消息撤回和离线文件提醒，因此进入阶段 21 实现。
...
- [x] 实现群组文件/图片：复用现有 TCP 文件传输通道，按在线成员逐个发送，UI 聚合显示投递状态。
- [x] 设计并实现群组文本离线成员补发：新增待投递队列、上线重试和消息去重策略。
- [x] 扩展群组附件离线补发：保存源文件路径、处理源文件丢失和重新发送接收确认。
- [x] 设计并实现群组端到端加密：先落地逐成员加密和加密补发保护，后续再考虑群密钥 epoch 轮换。
- [x] 明确暂缓剩余 P2：撤回、已读、文件传输加密。
- [x] 多文件/文件夹、断点续传已由用户明确要求进入阶段 20。
- **状态：** 进行中；聊天记录搜索、系统托盘、桌面通知、未读提醒强化、联系人/会话体验优化、日常沟通体验增强、私聊文本加密、群组/多人会话和群组高级能力已完成。

### 阶段 19：群组高级功能
- [x] 群组图片/文件：扩展文件请求载荷的群组元数据，发送方为每个在线成员创建独立 `FileId`，群组会话中聚合成一条附件消息。
- [x] 群组附件接收：接收方收到群组文件/图片请求时自动恢复群组会话，附件消息写入对应群组会话。
- [x] 群组附件状态：发送方显示在线成员投递汇总，接收/拒绝/失败/完成后更新聚合状态。
- [x] 群组文本离线补发：在群组附件状态基础上落地待投递表与上线重试。
- [x] 群组附件离线补发：复用待投递表，补齐源文件生命周期处理。
- [x] 群组端到端加密设计：在 HKDF 官方实现基础上复用一对一 E2EE 会话，完成逐成员加密和加密补发保护。
- **状态：** 已完成并验证；群组图片/文件、群组文本离线补发、群组附件离线补发和群组文本逐成员端到端加密均已完成。

### 阶段 20：多文件、文件夹传输与断点续传
- [x] 研究现有单文件、群组附件、离线补发和 TCP 文件流实现。
- [x] 扩展文件传输协议模型：批次 ID、文件项、相对路径、传输类型和续传 offset。
- [x] 扩展 TCP 文件流协议：兼容旧 header，新增 v2 header 支持从指定 offset 继续发送。
- [x] 实现多文件选择和文件夹选择入口；文件夹展开为相对路径文件项，保留目录结构。
- [x] 接收端对多文件/文件夹只确认一次，并为每个文件项计算保存路径与可续传 offset。
- [x] 发送端按文件项流式发送，传输中断后重新接受请求可从接收端已有字节继续。
- [x] 文件夹接收时创建必要子目录，阻止绝对路径和 `..` 路径穿越。
- [x] 私聊与群组附件链路均保持兼容；群组批量附件继续按成员逐个发送并聚合显示。
- [x] 更新 SQLite 文件记录字段与轻量迁移，记录批次、相对路径和已传输字节。
- [x] 补充序列化、数据库迁移、TCP 续传和文件夹路径安全测试。
- [x] 更新 README、`docs/protocol.md`、`docs/test-plan.md` 并执行 build/test 验证。
- **状态：** 已完成并通过自动化验证；仍建议用双机真实环境补充 100MB 文件夹和断网恢复验收。

### 阶段 21：已读回执、消息撤回与离线文件提醒
- [x] 研究现有私聊、群组、未读数、聊天历史、待投递队列和文件传输状态模型。
- [x] 设计协议载荷与枚举扩展：私聊/群组已读回执、撤回请求、离线文件提醒。
- [x] 扩展 SQLite：消息已读/撤回状态、群组成员已读计数、文件离线提醒记录或复用待投递队列。
- [x] 实现私聊已读：打开会话或收到当前会话消息时发送已读回执，发送方显示“未读/已读”。
- [x] 实现群组已读：按成员记录已读状态，发送方显示“已读 x/y”并区分群组和私聊。
- [x] 实现消息撤回：发送者可撤回自己消息，接收端同步标记为已撤回，本地历史不删除原记录但 UI 显示撤回提示。
- [x] 实现离线文件提醒：对离线成员发送文件/图片/批次时记录提醒，对方上线后收到文件待发送提醒，不误当作已传输完成。
- [x] 更新 UI：消息气泡状态文案、撤回入口/提示、离线文件提醒卡片或状态栏提示。
...
### 阶段 22：交付闭环与发布资料
- [x] 更新 `README.md`：补齐“已完成 / 进行中 / 暂未实现”能力清单、已知限制、发布路径与已验证命令。
- [x] 更新 `docs/test-plan.md`：区分自动化闭环、待人工验收项与发布验证步骤。
- [x] 新增或整理发布说明文档，明确普通 Release / Native AOT 命令、输出目录与已知 Avalonia DataGrid AOT 警告。
- [x] 更新 `task_plan.md`、`progress.md`、`next-development-plan.md` 的阶段状态，保留人工验收项但不再阻塞代码主线。
- [x] 串行执行 `dotnet build LanTalk.sln -v:minimal`、`dotnet test LanTalk.sln -v:minimal` 作为文档收口阶段基线。
- **状态：** 已完成
...
### 阶段 25：附件加密、导出与压缩管道
- [x] 为私聊和群组附件定义兼容旧端的加密元数据与传输封装，不破坏现有 `FileTransferRequest` / `FileTransferResponse` 流程。
- [x] 在文件传输客户端 / 服务端增加流式加解密能力，确保解密失败、认证失败有明确错误事件。
- [x] 增加会话消息导出（CSV / JSON）和文件传输记录导出（CSV），并提供异步 UI 入口。
- [x] 将 `ICompressor` 落地为可切换实现，至少提供 `Noop` 和 `GZip`，默认关闭以保持协议兼容。
- [x] 补充序列化、仓储、文件传输和消息服务测试，并执行 build / test 回归。
- **状态：** 已完成

### 阶段 26：通知抽象与开机自启
- [x] 抽象桌面通知接口，Windows 下优先尝试系统通知，不可用时回退到现有自绘 Toast。
- [x] 增加 Windows 开机自启平台服务，与设置保存联动，并保证关闭开关不影响普通启动。
- [x] 更新设置 UI、README 和平台限制说明。
- [x] 执行 build / test，并做 Windows 本机功能验证。
- **状态：** 已完成

### 阶段 27：最终验证、提交与推送
- [x] 串行执行最终 `dotnet build LanTalk.sln -v:minimal`。
- [x] 串行执行最终 `dotnet test LanTalk.sln -v:minimal`。
- [x] 执行普通 Release publish 并确认产物输出目录。
- [x] 执行 Native AOT publish，记录 DataGrid trim / AOT 警告是否仍存在。
- [ ] 按阶段边界提交并推送到远端。
- **状态：** 进行中
...
4. P2 剩余功能继续保持延期：多文件/文件夹、断点续传、撤回、已读、文件传输加密。
