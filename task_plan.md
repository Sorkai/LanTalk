...
### 阶段 11：多机真实验收与问题闭环
- [ ] 准备双机/三机验收环境：确认同一局域网、防火墙提示、默认端口占用情况。
- [ ] 验收 UDP 发现：A/B/C 同时启动、互相发现、刷新重新广播、关闭后 15 秒左右离线、重新上线恢复。
- [ ] 验收 TCP 私聊：A 发 B、B 回 A、未选择会话时收消息、离线用户发送失败提示、重启加载历史。
- [ ] 验收广播：A 广播给 B/C，接收端样式正确，广播历史保存。
- [ ] 验收文件传输：小文件、100MB 文件、接收方拒绝、传输中断、保存路径不存在或不可写、UI 不冻结。
- [ ] 将所有发现的问题分级记录：P0 阻塞演示、P1 影响体验、P2 后续增强。
- **状态：** 待开始

### 阶段 12：设置增强与端口可配置
- [ ] 将设置面板中的 UDP/TCP 消息/TCP 文件端口从只读展示升级为可编辑输入。
- [ ] 增加端口范围、重复端口、非法数字校验。
- [ ] 保存设置时提示端口变更需要重启监听或重启应用，并选择 MVP 内最稳妥的生效策略。
- [ ] 启动时检测端口占用，给出明确状态提示和日志。
- [ ] 为端口设置读写、非法配置规范化、端口冲突提示增加测试。
- **状态：** 待开始

### 阶段 13：网络与文件传输稳定性打磨
- [ ] 梳理 UDP/TCP/File 三条链路的异常提示，确保用户能看懂失败原因。
- [ ] 文件接收路径不可用时给出可恢复提示，避免静默失败。
- [ ] 文件传输中断后双方状态一致，数据库记录为失败，UI 不停留在“正在传输”。
- [ ] 对发送失败、对方离线、JSON 解析失败补充日志上下文。
- [ ] 保持文件传输继续使用流式分块和 `ArrayPool`，不引入断点续传等 P2 范围。
- **状态：** 待开始
...
### 阶段 15：后续 Backlog 收敛
- [ ] 根据阶段 11-14 的验收结果整理 P1/P2 待办，不直接扩大实现范围。
- [ ] 优先候选 P1：聊天记录搜索、桌面通知、系统托盘、跨网段手动添加 IP 的可行性评估。
- [ ] 明确暂缓 P2：永久群聊、多文件/文件夹、断点续传、撤回、已读、加密。
- **状态：** 待开始

## 当前项目状态下推荐的里程碑顺序
1. 先做阶段 11，因为当前代码功能链路已经齐，但还缺真实局域网验收证据；这一步最能暴露演示风险。
2. 再做阶段 12，因为端口可配置和端口冲突提示会直接影响多机运行稳定性。
3. 阶段 13 跟随验收问题做稳定性打磨，只修 MVP 范围内的可靠性问题，不引入断点续传等 P2 功能。
4. 阶段 14 在功能稳定后整理发布包、验收矩阵和课堂演示脚本。
5. 阶段 15 只收敛 backlog，作为后续迭代依据。
...
## 立即下一步
1. 实施阶段 11 的多机真实验收，优先收集 P0/P1 问题。
2. 运行 `dotnet build LanTalk.sln -v:minimal`。
3. 运行 `dotnet test LanTalk.sln -v:minimal`。
4. 若验收暴露端口、防火墙、文件保存或 UI 卡顿问题，先修 P0，再进入阶段 12。
5. 验收稳定后更新 `docs/test-plan.md` 与 README 的演示流程。
