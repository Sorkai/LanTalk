...
- [ ] 用真实端口占用场景手动验证冲突提示文案。
- [ ] 用双机/三机环境验证自定义发现网段是否能发现目标设备。
- **状态：** 单机实现已完成，等待端口占用和自定义网段实机验证。
...
## 立即下一步
1. 等用户准备双机/三机环境后，验证 `Auto` 与自定义 CIDR 网段发现效果。
2. 手动制造端口占用场景，确认启动提示和日志符合预期。
3. 继续验证文件传输中断、100MB 文件和防火墙拦截场景。
4. 根据真实验收结果更新 `docs/test-plan.md` 与 README 演示流程。
5. 进入阶段 14 整理课堂演示脚本与最终发布包。
