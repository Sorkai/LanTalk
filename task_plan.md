...
- [x] 实现日常沟通体验增强：图片消息便捷发送、缩略图预览、大图缩放、表情快捷输入和头像首字色块。
- [x] 实现临时群组、永久群组和多人会话：群组创建、成员选择、群组消息、永久群组本地持久化和启动恢复。
- [ ] 评估并实现下一个 P1：打包发布细节、开机自启或更完整的文件体验。
- [x] 实现私聊文本端到端加密开关：临时 ECDH 协商、AES-GCM 加密、会话指纹展示和关闭同步。
- [ ] 明确暂缓剩余 P2：群组文件/图片、多文件/文件夹、断点续传、撤回、已读、文件传输加密。
- **状态：** 进行中；聊天记录搜索、系统托盘、桌面通知、未读提醒强化、联系人/会话体验优化、日常沟通体验增强、私聊文本加密和群组/多人会话已完成。
...
3. 后续再评估打包发布、开机自启和更完整的文件体验。
4. P2 剩余功能继续保持延期：群组文件/图片、多文件/文件夹、断点续传、撤回、已读、文件传输加密。
